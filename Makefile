...
next-version: ## Print the version after the newest vX.Y.Z tag, or after package.json if untagged (BUMP=major|minor|patch).
...
# A to B" title — neither is expressible in the convention. Waved through.
...
# pushes is what triggers publish.yml (npm), and events raised by GITHUB_TOKEN
# do not start workflow runs — it would go silently dead.
tag-release: ## Tag HEAD as v<package.json version> and publish the GitHub Release.
	@set -eu; \
	tag="v$$($(pkg_version))"; \
...
churn-info: ## Print bump and line counts for FROM...TO (env: FROM, TO).
	...
	stat="$$(git diff --shortstat "$$FROM...$$TO" -- . ':!package-lock.json' ':!**/package-lock.json' 2>/dev/null || true)"; \
