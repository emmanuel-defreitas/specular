...
const EXPORTS = [
  { path: "@exegia/specular", title: "Define the surface", description: "defineSurfaces validates the shared config and preserves its types. litVars converts an angle to inline offsets for a layer named lit. Neither generates CSS.", href: "#basics", link: "Explore surfaces" },
  { path: "@exegia/specular/plugin", title: "Generate the CSS", description: "createPlugin(surfaces) generates Tailwind utilities and dark-mode presets at build time. Requires your surface config; React is optional.", href: "#setup-surfaces", link: "Set up the plugin" },
  { path: "@exegia/specular/merge", title: "Merge class names", description: "createCn(surfaces) returns a ready cn() helper. mergeConfig(surfaces) returns configuration for an existing tailwind-merge setup. Both use the same surfaces as the plugin.", href: "#merge", link: "Try class merging" },
  { path: "@exegia/specular/react", title: "Draw and move the rim", description: "Rim draws an overlay; usePointerLight supplies its angle. Optional tone helpers adapt it to images. React 19 is required; the Specular plugin is optional.", href: "#alive", link: "Explore React effects" },
]
...
bun add tailwind-merge@^3`}</Code>
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        {EXPORTS.map(({ path, title, description, href, link }) => (
          <article key={path} className="min-w-0 border-t border-stone-300/60 pt-5 dark:border-neutral-800">
            ...
            <p className="mt-2 text-sm leading-relaxed text-stone-600 dark:text-neutral-400">{description}</p>
